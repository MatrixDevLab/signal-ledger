...
correct; it only prevents a second model reading the same artifact from being
labelled independent evidence by omission.
"""
...
def evaluate(path: Path) -> dict[str, object]:
    ...
    for case in fixture.get("cases", []):
        ...
        independent = artifact != verifier and bool(verifier)
        expected = bool(case.get("expected_independent"))
        result = {"id": case.get("id"), "independent": independent, "expected": expected}
    ...
    return {
        ...
        "scope": "source identity only; no claim or outcome validation",
    }
